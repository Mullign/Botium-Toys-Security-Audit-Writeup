...
| Intrusion Detection System (IDS) | Detective; allows IT team to identify possible intrusions (abnormal traffic) quickly | X | High |
| Encryption keys | Deterrent; makes confidential information/data more secure (e.g., website payment transactions) | X | High |
| Backup files | Corrective; supports ongoing productivity in the case of an event; aligns to the disaster recovery plan | X | High |
| Password management systems | Corrective; password recovery, reset, lockout notifications | X | High |
| Antivirus software | Corrective; detect and quarantine known threats | X | High |
| Monitoring and maintenance | Preventative/corrective; required for legacy systems to identify and mitigate potential threats, risks, and vulnerabilities | X | High |
...
| Lockable cabinets/racks (for gear) | Preventative; increase integrity by preventing unauthorized personnel/individuals from physically accessing/modifying network infrastructure gear | X | High |
